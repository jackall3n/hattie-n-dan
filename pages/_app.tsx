...
function Header() {
  ...
  return (
    <div className="flex flex-col items-center relative bg-bottom bg-no-repeat bg-cover px-10 min-h-screen sm:min-h-0"
         style={{ backgroundImage: `url(/img_3.png)`, height: 600 }}>

      <div className="hidden text-center text-sm py-5 font-montserrat font-medium ">
        <span style={{ textTransform: "full-width" }}>IF YOU'RE READING THIS AND YOU'RE UNDER DURESS TO SPEAK TO AND/OR MARRY <i>DANIEL (THE USELESS CUNT) WILLIAMS</i> PLEASE CLICK THIS BUTTON 👉</span>
        <button className="bg-red-400 rounded-md px-3 py-1 text-base ml-3 uppercase"
                onClick={() => alert('Sorry help has not yet been implemented, please attempt to seek help another day')}>
          😢 Help me
        </button>
      </div>
      ...
      <div className="pt-10">
        <img
          ...
          src="https://fontmeme.com/permalink/211027/5dd84264fe337b75e0d52e0219200993.png" alt="handwriting-fonts"
        />
      ...
      <nav
        ...
        <NavItem title="The weekend" />
        <Spacer />
        <NavItem title="Accommodation" />
        <Spacer />
        <NavItem title="Transport" />
        <Spacer />
        <NavItem title="RSVP" />
      </nav>
    </div>
...
function NavItem({ title }) {
  return (
    <div className="uppercase font-medium text-sm tracking-wider text-black text-opacity-75">{title}</div>
  )
}
